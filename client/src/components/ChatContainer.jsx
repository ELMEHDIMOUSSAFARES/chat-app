    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg, index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img src={msg.image} alt='chat images' className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8'/>
            ): (
            ...
            <div className='text-center text-xs'>
              <img src={msg.senderId === authUser._id ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="profile picture"
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
          <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>e.key === "Enter" ? handleSendMsg(e) : null} type="text" placeholder='Send a message...'
        ...
        <img onClick={handleSendMsg} src={assets.send_button} alt="send button" className='w-7 cursor-pointer'/>
      </div>
    </div>
  ): ( 
      // No selected user view
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/6 max-md:hidden'>
      <img src={assets.logo_icon} alt="logo icon" className='max-w-16'/>
      <p className='text-lg font-normal text-amber-50'>Chat Anytime!</p>
    </div>
  )
}
